...
export default function Research(props: Props): JSX.Element {
	...
	return (
		<div className={css.container}>
			<span className={css.alignCenterFlex}><span className={css.label}>Severity:</span> {getSeverityImage(props.data?.jfrogResearchSeverity ?? ISeverity.Unknown, true)} {props.data?.jfrogResearchSeverity}</span>
			<Divider/>
			...
			<div className={css.group}>
				...
				{increasesRisk && increasesRisk.length > 0 &&
					<>
						<span className={css.alignCenterFlex} style={{ color: '#F14C4C' }}><img src={moreRiskIcon}/> Increases the risk level:</span>
				...
				{decreasesRisk && decreasesRisk.length > 0 &&
					<>
						<span className={css.alignCenterFlex} style={{ color: '#3EB065' }}><img src={lessRiskIcon}/> Decreases the risk level:</span>
						<ul className={css.bulletList}>
