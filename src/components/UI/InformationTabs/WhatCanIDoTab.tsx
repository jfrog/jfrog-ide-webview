...
export default function WhatCanIDoTab(props: WhatCanIDoTabProps): JSX.Element {
    ...
    const directDependanciesNames = (props.impactGraph?.root.children ?? []).map(node => node.name.split(':')[0])
	...
	<div className={css.container}>
		...
		{props.remediation && props.remediation.length > 0 && <Collapse
			header={<h1>Patch the code {[PageType.Eos].includes(props.pageType) && <span className={css.recommendedLabel}>Recommended</span>}</h1>}
			content={
				<div>
					<p><b>Remediation</b></p>
		...
		{props.fixedVersion && props.fixedVersion.length > 0 && <Collapse
			header={<h1>Update direct dependency <span className={css.recommendedLabel}>Recommended</span></h1>}
			content={
				<div>
					<p className={css.alignCenterFlex}>
						{isDirectDependency ?
							<span style={{ display: 'flex', gap: 8, flexDirection: 'column' }}>
								<div>
									<span className={css.text}>Update the following:
									</span> <span>{directDependanciesNames.join(", ")}</span>
								</div>
								<div>
									<span className={css.text}>Fix version:
									</span> <span>{props.fixedVersion.join(", ")}</span>
								</div>
							</span> :
							<span>
								<span
									className={css.text}>Update the following:
								</span> <span>{directDependanciesNames.join(", ")}</span>
							</span>}
					</p>
				</div>
                }/>}
		{props.fixedVersion && props.fixedVersion.length > 0 && !isDirectDependency && <Collapse
				...
				<div>
					<p className={css.alignCenterFlex}>
						<span style={{ display: 'flex', gap: 8, flexDirection: 'column' }}>
							<div>
								<span className={css.text}>Update the following:
								</span> <span>{props.component}</span>
							</div>
							<div>
								<span className={css.text}>Fix version:
								</span> <span>{props.fixedVersion.join(", ")}</span>
		...
		{[PageType.Eos, PageType.Secrets].includes(props.pageType) && <Collapse
			header={<h1>Suppress the finding</h1>}
			content={
				<div>
					{/* eslint-disable-next-line react/jsx-no-comment-textnodes */}
					<p>Add <code>jfrog-ignore</code> comment above the vulnerable line to suppress it</p>
					<Markdown
						text={'```javascript \n' +
                            '// Javascript Example \n' +
                            'export: (req, res) => {\n' +
