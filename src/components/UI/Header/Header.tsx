...
import { IAnalysisStep, IDependencyPage, IEosPage, IIaCPage, ISecretsPage, PageType } from "../../../model"
...
const LocationSpan = (props: { location: IAnalysisStep }): JSX.Element => (
	<span className={css.locationLabel}><b>Location:</b> {props.location.file}</span>
)
...
const CveInformation = (props: { data: IDependencyPage }): JSX.Element => (
	<div className={css.cveInformationContainer}>
		...
		<span className={css.cveLabel}>CVSS: &nbsp;{
			props.data.cve?.cvssV2Score && (<div className={cveCss.redBox}>{props.data.cve.cvssV2Score} (v2)</div>)
		}
		</span>
		<span className={css.cveLabel}>JFrog severity rank: {getSeverityImage(props.data.extendedInformation?.jfrogResearchSeverity ?? ISeverity.Unknown, true)}</span>
	</div>
)

export default function Header(props: Props): JSX.Element {
	const showLocationInPages = [PageType.IaC, PageType.Secrets, PageType.Eos].includes(props.pageData.pageType as PageType)
	const showCVEInfoInPages = PageType.Dependency === props.pageData.pageType
	const showJFrogResearchIcon = (props.pageData as IDependencyPage).extendedInformation !== undefined
		...
		<div className={css.content}>
			<div className={css.titleContainer}>
				...
				<h6 className={css.flexCenter}>
					{props.text}
					{showJFrogResearchIcon && <img src={jfrogResearchIcon}/> }
					{showIsApplicable && <img src={applicableIcon}/> }
				</h6>
			</div>
			{showCVEInfoInPages && <CveInformation data={props.pageData as IDependencyPage}/>}
			{showLocationInPages && <LocationSpan location={(props.pageData as ISecretsPage).location}/>}
		</div>
	</div>
    )
}
